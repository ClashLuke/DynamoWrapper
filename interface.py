...
to_get = {'dtype': ['data_type', ''],
          ...
          # [item, validated_item], [using_gsi, using_primary, chunk, all], [item, validated_item]
          'segments': ['threads', 1],
...
def overwrite_variable(args, default_map, arg_index, map_index):
    ...
    If the args[arg_index] does not exist in default_map, a declarative exception is raised.
...
def extract_args(**kwargs):
    ...
    if args.get('dtype', False):
        args['table_name'] = ''.join([args['dtype'], args['ttype']])
    else:
...
def init(stage, **kwargs):
    processed_args = extract_args(**kwargs)
    table_name = get_table_name(stage, processed_args['table_name'])
    processed_args['table_name'] = table_name
